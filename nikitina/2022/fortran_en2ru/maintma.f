     ...
     *          tenvy(20)
      ...
      DATA Mp1 /'íêÄïìçéÇ','íêìîÄçéÇ','äìÑêüÇñÖÇ','äìíÖèéÇ',
     *'ÅÄÅÄüç',' ','ëÄÇóÖçäé','ëàçÑûäéÇÄ',''/
      DATA MshHy/
     * '  éÅ	 Hì    BàTOK    ÑATA	       BPEMü   ',
     * '     X	     ì	       Z	VX	   Vì	      VZ'/
      DATA Mshid/
     * '   G	  GÅé	  GCA	  P	P ìÑ	DVT',
     * '    XT       ìT         SCA',
     * '                                      ÑATA àÑ'/
      DATA Mid615 /
     * '    G	   G KAèC     P    P ìÑ   DVT	ÑATA àÑ'/
      DATA KyX/8137/
      DATA KBCyT /16/
      DATA nz0/14/
      DATA nachzoh /0/
...
c=================================================
 ...
 1000 WRITE (*,1)
      KBBHy=0
    1 FoRMAT(15/50X,' >>> BEPCàü éT 10.01.2006 <<<'//
     * 10X,'	            C A Ñ à K  -  í å Ä	       '/
     *10X,' ( CèìCKéBéâ ABTéMATàáàPéBAHHõâ ÑàAãéÉéBõâ KéMèãEKC )'//
     * '    óTé HìÜHé? - èPéÉHéá          (1)'/
     * '                 CèìCK ëéûá íåÄ   (2)'/
     * '                 CèìCK èêéÉêÖëë å (3)'/
     * '                 éTHéë. ÑBàÜEHàE  (4)'/
     ...
     * '    BõÅàPAâTE :')
      ...
      Go To 1000
 1022 WRITE (*,1023)
 1023 FoRMAT(/'    óàCTàTú APXàB (1), HET (0) ?')
 ...
 1024 Al(II)=0.
      ...
      Go To 1000
   10 WRITE (*,3)
    3 FoRMAT(/'    Bõ, KAÜETCü, XéTàTE áAKéHóàTú PAÅéTì C',
     * '  KéMèãEKCéM ?'/
     * '    BBEÑàTE (0), ECãà ìBEPEHõ àãà óTé-HàÅìÑú ñEãéE, ECãà',
     * '  HET')
      CALL vvodi('   -->',K)
      ...
      close(72)
      WRITE (*,5)
    5 FoRMAT(/'      HE áÄÅìÑúíÖ èéëåéíêÖíú ãàCTàHÉ (list.rez)!'/
     *	'	     ... ÅãAÉéÑAPû áA BHàMAHàE .'/)
      SToP
    4 Go To 1000
C	CèìCK 732
  200 WRITE (*,400)
  400 FoRMAT(/5X,'    * CèìCK ëéûá íåÄ *'//
     * '    BõÅàPAâTE: îàáàKA+îéPMõ        (1)'/
     *        11X,'    îéPMõ               (2)'/
     *        11X,'    APXàB îàáàKà à îéêå (3)'/
     *        11X,'    áAèàCú îéêå Ç ÅÑ    (4)'/
     *        11X,'    B õ X é Ñ	          (0)')
  ...
  202 WRITE (*,1600)
 1600 FoRMAT(/'    áAÑAHàE ÅPATú: C ùKPAHA  (1)'/15X,
     * '    àá APXàBA (2)	(C KTã (-2))'/15X,'    TEKìôàM   (3)')
      CALL vvodi('   -->',K)
      ...
      IF(K.EQ.2) Go To 270
      WRITE (*,272)
  272 FoRMAT(/'               KATAãéÉ APXàBA áAÑAHàâ'//
     * '                ä-¢Æ              1-©            1-Ô'/
     * '    é°´   Ç†‡   ·‚‡Æ™   îÆ‡¨†    ¢®‚Æ™  èÆ‡Ê®Ô   ‚´£ ')
      Do 273 I=1,6
      L=360+30*(I-1)-1
  ...
  274 FoRMAT(I3'.',I5,I7,F10.1,I8,I7,I8)
      write(*,*)
  270 CALL vvodi('   HéMEP éÅãACTà (1-6) --> ',Nobl)
...
c      Nbb=nint(Al(IH+7))
  ...
  203 KolCTz=1
      JNAME=0
      CALL vvodi('    HéMEP BAPàAHTA? --> ',NzPC)
      IF(NzPC.EQ.KyX) Go To 1000
  207 WRITE (*,1601)
 1601 FoRMAT('    BBEÑàTE HéMEP îéPMõ (14,510,511,524,514,521)')
     ...
     *	AND.ABS(foPMA(KolCTz)-524.).GT.0.5) Go To 2050
      ...
      WRITE (*,'(/a)') '    KTé ÅìÑET èéÑèàCõBATú? (ÇÇÖÑàíÖ çéåÖê)'
      do i=1,9 
 ...
 2058 Call VVodi('',Nname)
      ...
      JNAME=1
 2050 WRITE (*,1602)
 1602 FoRMAT('    BBEÑàTE HéMEP 1-â TEãEÉPAMMõ')
      CALL vvodi('   -->',NTlg0(KolCTz))
      IF(NTlg0(KolCTz).EQ.KyX) Go To 1000
  205 WRITE (*,1603)
 1603 FoRMAT(/'    áAÑAHàE BCE (0) àãà HET (1) ?')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 10
      IF(K.EQ.0) Go To 204
      IF(K.NE.1) Go To 205
      ...
      WRITE (*,1604)
 1604 FoRMAT(/'    C MEHü, éÑHAKé, ÑéBéãúHé ...')
      Go To 204
  206 KolCTz=KolCTz+1
      Go To 207
  204 WRITE (*,2041)
 2041 FoRMAT(/'    BEPHé (0), èPéBEPKA (1), áAèàCú (2),',
     *	  ' àHAóE (3)')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 1000
 ...
 2056 FoRMAT(I8,F9.1,I10,I9,I10,7X,a9)
  208 CoNTINUE
      Go To 204
  210 CALL vvodi('   HéMEP éÅãACTà (1-6) --> ',Nobl)
      IF(Nobl.EQ.KyX) Go To 1000
      IH=360+30*(Nobl-1)-1
...
c      Al(IH+7)=Nbb
  ...
  263 Al(L+4)=NTlg0(I)
      ...
      CALL WRTAPE(Al,1024,4,5,NAchzoH)
      WRITE (*,532)
  532 FoRMAT(/'    +++ èPéòãA áAèàCú B APXàB KPC +++')
      Go To 204
 2110 NpPHAch=100000
      NpPKoH=0
      Do 212 I=1,KolCTz
  ...
  212 CoNTINUE
      ...
      WRITE (*,1609)
 1609 FoRMAT(///'    BHàMAHàE, HìÜEH CãàòKéM ÑãàHHõâ',
     *	' èPéÉHéá !!!'/'         CKéPPEKTàPìâTE áAÑAHàE !'/)
      Go To 200
  213 WRITE (*,1610)
 1610 FoRMAT(/'    èPéÉHéá CóàTATú (0) àãà ÅPATú ÉéTéBõâ (1) ?')
      CALL vvodi('    --> ',KEpPog)
      IF(KEpPog.EQ.KyX) Go To 1000
      IF(KEpPog.NE.1) Go To 214
      IF(KlgpP.NE.0) Go To 215
      WRITE (*,2157)
 2157 FoRMAT(/'        ÑA HE CóàTAãà Bõ EôE èPéÉHéá !!!')
      Go To 213
  ...
  215 KgAPM=1
      ...
      WRITE (*,2151)
 2151 FoRMAT(/'      BHàMAHàE, HéMEP BàTKA Hì > 1-Éé BàTKA CóETA !'/)
...
c      call write_frm_ora
	isys=SYSTEM("StDB.exe") 
      Go To 200
  920 WRITE (*,921)
  921 FoRMAT(/'    óàCTàTú APXàB îàáàKà (1), HET (0) ?')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 1000
      IF(K.NE.1) Go To 200
  ...
  100 WRITE (*,8)
   ...
   21 FoRMAT(/'    èPéBEPKA HìÜHA (1), HET (0), àHAóE (2)')
  500 FoRMAT(/'    KATAãéÉ HìÜEH (1), HET (0) ?')
  512 FoRMAT('    BCE (0), EôE (1), TAÅã (2) ?')
  514 FoRMAT('    áAèàCõBATú B APXàB (1), HET (0) ?')
 ...
 9801 FoRMAT(/,5X,' ***  C A Ñ à K  ***   ',/'F=',F6.1,
     *	'  F81=',F6.1,'	AP=',F5.1,'  SéPÅ=',F7.5//A/
     ...
     * ':',F6.3,' (',F9.3,')'//a/3F9.3,3F11.5)
      ...
      WRITE (*,31)
   31 FoRMAT(/
     *'    CóàTATú èPéÉHéá HA áAÑAHHéE K-Bé BàTKéB       (1)'/
     *'    àãà C áAÑAHHéÉé BàTKA HA áAÑAHHéE K-Bé BàTKéB (2)'/
     *'    àãà C MéÑEãàPéBAHàEM MAHEBPéB                 (3)')
      CALL vvodi('   -->',KpP)
      IF(KpP.EQ.3) Go To 5301
      IF(KpP.EQ.KyX) Go To 1000
      IF(KpP.NE.1) Go To 52
   34 WRITE (*,32)
   32 FoRMAT(/'    BBEÑàTE KéãàóECTBé BàTKéB èPéÉHéáA ( <= 200 )')
      CALL vvodi('   N = ',N)
      IF(N.EQ.KyX) Go To 1000
   35  WRITE (*,33)
   33 FoRMAT('    BEPHé (0), EôE PAá (1)')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 1000
      IF(K.EQ.0) Go To 53
      IF(K.EQ.1) Go To 34
      Go To 35
   52 WRITE (*,36)
   36 FoRMAT(/'    èPéÉHéá CóàTATú C BàTKA')
      CALL vvodi('   N = ',N)
      IF(N.EQ.KyX) Go To 1000
      WRITE (*,37)
   37 FoRMAT('    KéãàóECTBé BàTKéB èPéÉHéáA ( <= 200 )')
      CALL vvodi('   DN = ',N1)
   39 WRITE (*,38)
   38 FoRMAT('    BEPHé (0), EôE PAá (1)')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 1000
      IF(K.EQ.0) Go To 53
      IF(K.EQ.1) Go To 52
 ...
 5301 N=1
   53 WRITE (*,40)
   40 FoRMAT(/'    èéKAáõBATú XéÑ èPéÉHéáA (1) àãà HET (0) ?')
      CALL vvodi('    --> ',KBud)
      IF(KBud.EQ.KyX) Go To 1000
      WRITE (*,7300)
 7300 FoRMAT(/'      àÑTà (1), ÑéèéãHàTEãúHõE PEÜàMõ (0) ?')
      CALL vvodi('    --> ',JM)
      IF(JM.EQ.1) Go To 7901
      IF(JM.NE.0) Go To 1000
      WRITE (*,7902)
 7902 FoRMAT(/'    C PACóETéM áéH BàÑàMéCTà	   (1)'/
     *	'    C PACóETéM "ñì" HA KAÜÑéM BàTKE (2)'/)
      CALL vvodi('    --> ',JM)
 ...
 7901 KSoPb=0
      WRITE (*,41)
   41 FoRMAT(/'    *** àÑET PACóET èPéÉHéáA ***'/)
      Kid=1
      KHy=1
      NB1=NB
      Nch1=Nch
...
c-------------------------------------
 ...
 4100 KlgpP=1
   ...
   61 FoRMAT(/'    >*< Bõ èõTAãàCú CóàTATú èPéÉHéá ÑAãúòE,óEM',
     *	  ' ùTé BéáMéÜHé >*<'/'       èéCãEÑHàâ BàTéK ',F6.0,'(',
     *	F3.0,')   ',F3.0,'.',F3.0,'.',F5.0)
   60 WRITE (*,62)
   62 FoRMAT(/'    BõÑABATú èPéÉHéá: HA ùKPAH',14X,'(1)'/
     * 18X,'    Ç ãàëíàçÉ',13X,'(2)'/18X,'    B TEKìôàÖ Hì',10X,'(3)'/
     * 18X,'    KéHEñ áAÑAóà èPéÉHéáA (0)')
      CALL vvodi('    --> ',K)
      IF(K.EQ.KyX) Go To 1000
      ...
      Go To 60
   67 WRITE (*,64)
   64 FoRMAT(/'    ÇõÑATú C BàTKA HéMEP (BBEÑàTE)')
      CALL vvodi('    --> ',NHAch)
...
c Orbit has been found
   ...
   65 CoNTINUE
      WRITE (*,68)
   68 FoRMAT(/'    --- Ñãü ùTéÉé BàTKA ÑAHHõX HET ---')
      Go To 67
  ...
  967 XPHy(I+12)=PEz(IHAch,I+8)
      WRITE (*,968)NHAch
  968 FoRMAT(//'    Hì BàTKA ',I5,' èEPEèàCAHõ B TEKìôàE',
     *	  ' Hì èPéÉHéáA '/)
      Go To 60
   66 WRITE (*,69)
   69 FoRMAT(/'    HA CKéãúKé BàTKéB BõÑATú (BBEÑàTE)')
      CALL vvodi('   -->',NBud)
   ...
   71 FoRMAT(/'    óEÉé áAXéTEãà, ì MEHü CTéãúKé HETì.'/
     *	'    MéÉì BõÑATú ',I3,' BàTKéB èé ',F5.0,'-â BKãûóàTEãúHé')
      Go To 66
   70 Do 72 M=IHAch,J
   72 CALL VydPR1(M,K)
      Go To 60
 ...
 7001 FoRMAT(/5X,'    * CèìCK èêéÉêÖëë å *'//
     * '    BõÅàPAâTE: îàáàKA              (1)'/
     *	      11X,'    îéPMõ               (2)'/
     *        11X,'    APXàB îàáàKà à îéêå (3)'/
     *	      11X,'    áÄèàëú Ç ÅÑ         (4)'/
 ...
 7010 WRITE (*,7011)
 7011 FoRMAT(/'    BBEÑàTE áAÑAHàE HA PACóET :'/)
      CALL vvodi('   HéMEP BAPàAHTA',NzPC)
      CALL vvodi('   HéMEP BàTKA CóETA',NHAchB(1))
      WRITE (*,28)
   28 FoRMAT(/'    BEPHé (0), HET (1) ?')
      CALL vvodi('   -->',L)
...
c output NU, ID ,MODES and TASK to file for printing     
  ...
  605 WRITE (*,41)
     ...
     *	  F,F81,AP,Kd,NKoH,No520,NHy,Nsh,Nbb,Nob)
      KlgpP=1
 6060 WRITE (*,606)
  606 FoRMAT(/'    >*< èPéÉHéá ÉéTéB, CóàTAû CèìCK >*<'/)
...
c calculation of forms      
      ...
      KfoPM1=KfoPM*0.1
      MTf='   '
      IF(KfoPM.EQ.5141.oR.KfoPM.EQ.5241) MTf='-ÅC'
      IF(KfoPM.EQ.5142.oR.KfoPM.EQ.5242) MTf='-ìC'
      WRITE (*,5566) nint(foPMA(LK)),MTf
 5566 FoRMAT(/'        * îéPMA ',i4,A3,' *'/)
      MbC=0
      IF(KfoPM.NE.14.AND.
 ...
 1111 IF(MbC.EQ.1) Go To 612
      WRITE (*,609)LK
  609 FoRMAT(/'    B H à M A H à E , CTPéKA ',I2,
     *	      ' HE éÅPAÅATõBAETCü !')
      Go To 5000
  612 NHAch=NHAchB(LK)
      KgoTB=0
...
c      Tlg=NTlg0(LK)+100*Nsh
  ...
  614 NBpP=nint(PEz(IB,1))
      ...
      WRITE (*,6141)NHAch
 6141 FoRMAT(/'    HET èPéÉHéáA Ñãü BàTKA ',I5)
      Go To 5000
 6140 IB=IB+1
      Go To 614
